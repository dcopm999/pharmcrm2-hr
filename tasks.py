...
def coverage(c):
    ...
    """
    c.run("coverage run --source pharmcrm2-hr runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/pharmcrm2-hr.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ hr")
...
def lint(c):
    ...
    """
    c.run("flake8 pharmcrm2-hr tests")
...
def release(c, bumpsize=""):
    ...
    c.run(
        "git tag -a {version} -m 'New version: {version}'".format(
            version=pharmcrm2_hr.__version__
        )
    )
    c.run("git push --tags")
